...
void Matrix4x4::setRow(unsigned int index, const Vector3 &row) {
...
void Matrix4x4::setOrigin(const Vector3 &origin) {
	mData[12] = origin.x();
	mData[13] = origin.y();
	mData[14] = origin.z();
	mData[15] = 1.0f;
...
Matrix4x4& Matrix4x4::translate(const Vector3& offset) noexcept {
...
Matrix4x4& Matrix4x4::lookAt(const Vector3& eye, const Vector3& center, const Vector3& up) noexcept {
	const Vector3& zAxis = -(center - eye).normalized();
